...
const genreNeighbors: Record<string, string[]> = {
...
function toMembershipBands(value: number): MembershipBands {
	const normalized = Math.max(0, Math.min(100, value))
	const low = normalized <= 25 ? 1 : normalized >= 55 ? 0 : (55 - normalized) / 30
	const medium =
		normalized <= 30 || normalized >= 80
			? 0
			: normalized <= 55
				? (normalized - 30) / 25
				: (80 - normalized) / 25
...
function getMovieScoreValue(movie: MovieWithValues, variableId: number) {
	return movie.variableValues.find(item => item.variableId === variableId)?.value ?? null
}

function getTermMembership(
	variable: VariableWithTerms,
	termKey: string,
	...
	switch (variable.kind) {
		...
		case VariableKind.GENRE:
			if (movie.genres.includes(term.label)) return 1
			return genreNeighbors[term.label]?.some(item => movie.genres.includes(item))
				? 0.72
				: 0.22
		case VariableKind.AUDIENCE:
			if (term.key === 'any') return 0.65
			if (term.key === 'family') return movie.familyFriendly ? 1 : 0.18
			if (term.key === 'adult') return movie.familyFriendly ? 0.45 : 1
			return 0
	}
}
...
function evaluateCompatibility(
	variable: VariableWithTerms,
	selectedTermKey: string | undefined,
	movie: MovieWithValues,
) {
	if (!selectedTermKey) {
		return {
...
function getBandValue(level: CompatibilityLevel, bands: MembershipBands) {
	switch (level) {
...
function defuzzify(ruleShapes: Array<{ strength: number; rule: RuleWithConditions }>) {
	...
	for (const x of domain) {
		const aggregated = Math.max(
			...ruleShapes.map(({ strength, rule }) =>
				Math.min(
					strength,
					evaluateShape(
						rule.outputFunctionType,
						x,
						rule.outputA,
						rule.outputB,
						rule.outputC,
...
export function buildRecommendations(params: {
	movies: MovieWithValues[]
	variables: VariableWithTerms[]
	...
	return filteredMovies
		.map(movie => {
			const compatibilities = activeSearchVariables.map(variable =>
				evaluateCompatibility(variable, payload.inputs[variable.key], movie),
			)
			const compatibilityMap = Object.fromEntries(
				compatibilities.map(item => [item.variableKey, item]),
			)
			...
			const ruleShapes = rules
				...
				.map(rule => {
					...
					const baseStrength =
						rule.connector === RuleConnector.AND
