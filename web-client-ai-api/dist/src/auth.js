...
function parseCookies(header) {
    if (!header)
        return {};
    return Object.fromEntries(header.split(';').map(chunk => {
...
function signSessionValue(login) {
    return crypto
...
export function isValidCredentials(login, password) {
    return (secureCompare(login, config.adminLogin) &&
...
export function getSessionUser(req) {
    ...
    if (!login || roleValue !== 'admin' || !signature)
        return null;
    const expectedSignature = signSessionValue(login);
    if (!secureCompare(signature, expectedSignature))
        return null;
    if (!secureCompare(login, config.adminLogin))
        return null;
    return {
