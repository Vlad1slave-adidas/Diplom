...
import { clearAuthCookie, createAuthCookie, getSessionUser, requireAdmin, isValidCredentials, } from './auth.js';
...
import { movieSchema, ruleSchema, searchSchema, termSchema, variableSchema, } from './validators.js';
const app = express();
app.use(cors({
...
app.get('/api/public/filters', async (_req, res) => {
    ...
    const genres = Array.from(new Set(movies.flatMap(movie => movie.genres))).sort((a, b) => a.localeCompare(b, 'ru'));
    const countries = Array.from(new Set(movies.map(movie => movie.country))).sort((a, b) => a.localeCompare(b, 'ru'));
    const years = Array.from(new Set(movies.map(movie => String(movie.year)))).sort((left, right) => Number(right) - Number(left));
    res.json({
        variables: variables
...
app.get('/api/public/movies/:slug', async (req, res) => {
    ...
    const selectedInputs = Object.fromEntries(Object.entries(req.query).map(([key, value]) => [key, String(value)]));
